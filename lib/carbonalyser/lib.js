...
let isDebug = null;
isInDebug = async () => {
    if ( isDebug === null || isDebug === undefined ) {
        isDebug = (await getPref("debug"));
    }
    return isDebug;
}

let blackList = [];
let whitelist = [/.*getPref.*/];
printDebug = async (msg) => {
    if ( !(await isInDebug()) ) {
        return;
    }
    let accepted = true;
...
preferencesChange = async (changes, areaName) => {
    if ( areaName == "local" ) {
...
init = () => {
    obrowser.storage.onChanged.addListener(preferencesChange);
}

end = () => {
    // Under firefox 98.0.1 context of popup is destructed before backend receive
    // message of remove listener.
    obrowser.storage.onChanged.removeListener(preferencesChange);
}
