...
createSumOfData = (rawdata, type, tsInterval=60, byOrigins=undefined) => {
    ...
    for(const origin in rawdata) {
        if ( byOrigins === undefined || (byOrigins instanceof [] && byOrigins.indexOf(origin) !== -1) || (byOrigins instanceof "" && origin === byOrigins)) {
 ...
 */
compileBytes = (rawdata, separator, newline) => {
...
createStatsFromData = (rawdata) => {
    const bytesDataCenterUnordered = createSumOfData(rawdata, 'datacenter');
    let bytesNetworkUnordered = createSumOfData(rawdata, 'network');
