 ...
 */
getParameters = () => {
    let p = JSON.parse(localStorage.getItem('parameters'));
    if ( p === null ) {
        p = {selectedRegion: null, regions: {}, lastRefresh: null, refreshInterval: 3600 * 1000};
...
setRefreshInterval = (interval) => {
    ...
    setParameters(p);
    runtime.sendMessage({action: "restartCIUpdater"});
}

/**
 * Get the refresh interval.
 * TODO: analysis.carbonIntensity.refreshMs
 */
getRefreshInterval = () => {
    return getParameters().refreshInterval;
}
