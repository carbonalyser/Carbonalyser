...
getOrCreateRawData = async (origin) => {
    let rv = await browser().storage.local.get('rawdata');
    rv = undefined === rv ? {} : JSON.parse(rv);
...
incBytesPerOrigin = async (classType, origin, bytes) => {
    ...
    console.trace();
    await browser().storage.local.set({rawdata: JSON.stringify(rawdata)});
}
 ...
 */
getParameters = async () => {
    let v = await browser().storage.local.get('parameters');
    if ( v === undefined ) {
        v = null;
    }
    let p = JSON.parse(v);
    if ( p === null ) {
        ...
        console.trace();
        await browser().storage.local.set({parameters: JSON.stringify(p)});
    }
    return p;
}
...
setParameters = async (parameters) => {
    console.trace();
    await browser().storage.local.set({parameters: JSON.stringify(parameters)});
}
