 ...
 */
initLibStats = () => {
    carbonIntensityFactorIngCO2PerKWh = getRegions();
}
...
getOrCreateRawData = (origin) => {
    let rv = chrome.storage.local.get('rawdata');
...
incBytesPerOrigin = (classType, origin, bytes) => {
    ...
    chrome.storage.local.set({rawdata: JSON.stringify(rawdata)});
}
 ...
 */
getParameters = () => {
    let v = chrome.storage.local.get('parameters');
    if ( v === undefined ) {
        v = null;
    }
    let p = JSON.parse(v);
    if ( p === null ) {
        p = {regions: {}, lastRefresh: null};
        chrome.storage.local.set({parameters: JSON.stringify(p)});
    }
    return p;
}
...
setRefreshInterval = (interval) => {
    setPref("analysis.carbonIntensity.refreshMs", interval);
}
 ...
 */
getRefreshInterval = () => {
    return getPref("analysis.carbonIntensity.refreshMs");
}
 ...
 */
setParameters = (parameters) => {
    chrome.storage.local.set({parameters: JSON.stringify(parameters)});
}
