...
/**
 * Get the current country of user (as carbonalyser internal).
 */
fetchCurrentCountry = () => {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "https://ipinfo.io/", false);
    xhr.setRequestHeader("Accept", "application/json");
    xhr.send();
    if ( xhr.status === 200 ) {
        const CC = JSON.parse(xhr.responseText).country;
        if ( alpha2TOname[CC] === undefined ) {
            return DEFAULT_REGION;
        } else {
            return alpha2TOname[CC];
        }
    } else {
        console.warn("cannot fetch the current country " + xhr.response + " " + xhr.responseText + " ");
        return DEFAULT_REGION;
    }
}
 ...
 */
fetchCurrentCountryWhenNeeded = async () => {
    let selectedRegion = await getPref("analysis.selectedRegion");
    if ( selectedRegion === undefined || selectedRegion === DEFAULT_REGION ) {
        if ( await getPref("daemon.fetchCurrentLocation") ) {
            selectedRegion = fetchCurrentCountry();
