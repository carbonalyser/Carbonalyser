...
injectEquivalentIntoHTML = async (stats, computedEquivalence) => {
    ...
    let f = (computedEquivalence.kWhTotal * electricity).toFixed(3);
    if ( f == 0 ) {
        document.getElementById('kWhTotalValue').textContent = "0";
    } else {
        document.getElementById('kWhTotalValue').textContent = f.toString();
    }
