...
injectEquivalentIntoHTML = (stats, computedEquivalence) => {
    ...
    document.getElementById('equivalenceTitle').textContent = getBrowser().i18n.getMessage('equivalenceTitle', [computedEquivalence.duration.toString(), megaByteTotal, computedEquivalence.kWhTotal.toString(), computedEquivalence.gCO2Total.toString()]);
}
...
computeEquivalenceFromStatsItem = async (stats) => {
    let res = await getBrowser().storage.local.get('duration');
    res.duration = undefined === res || res.duration === undefined ? 0 : res.duration;
