...
autoSelectCurrentRegion = async () => {
    console.warn("autoSelectCurrentRegion")
    ...
    for(const regionName in regions) {
        ...
        console.warn("test if user " + JSON.stringify(currentPosition) + " is in " + regionName);
        if ( d3.geoContains(regionObject, [currentPosition.longitude, currentPosition.latitude]) ) {
            console.warn("user could be in " + regionName + " need more checks");
        }
    }
}
 ...
 */
fetchCurrentPositionIfEmpty = async () => {
    let selectedRegion = await getPref("analysis.selectedRegion");
    if ( selectedRegion === undefined ) {
        await setSelectedRegion(DEFAULT_REGION);
    }
    console.warn("todo fetch position from the browser instead (setSelectedRegion('regionCurrentPosition'))");
}
