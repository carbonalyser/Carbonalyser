...
injectEquivalentIntoHTML = async (stats, computedEquivalence) => {
    ...
    document.getElementById('OneHourBulbNumberValue').textContent = computedEquivalence.OneHourBulbNumber.toString();
}
...
computeEquivalenceFromStatsItem = async (stats) => {
    ...
    res.duration = duration === undefined ? 0 : duration;
    res.kWhDeviceTotal = res.duration * (await getPref("general.kWhPerMinuteDevice"));
    ...
    res.chargedSmartphones = Math.round(res.gCO2Total / (await getPref("general.GESgCO2ForOneChargedSmartphone")));
    res.OneHourBulbNumber = res.kWhTotal / (await getPref("general.BulbConsumptionKWh"));
    return res;
}