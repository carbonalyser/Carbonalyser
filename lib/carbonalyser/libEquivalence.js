...
computeEquivalenceFromStatsItem = (stats) => {
    let res = {};
    res.duration = chrome.storage.local.get('duration');
    ...
    res.GESDeviceTotal = res.kWhDeviceTotal * carbonIntensityFactorIngCO2PerKWh[getSelectedRegion()].carbonIntensity;
...
updateEquivalence = (stats) => {
    data.equivalence = computeEquivalenceFromStatsItem(stats);
