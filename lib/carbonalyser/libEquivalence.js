...
injectEquivalentIntoHTML = (stats, computedEquivalence) => {
    ...
    document.getElementById('equivalenceTitle').textContent = browser().i18n.getMessage('equivalenceTitle', [computedEquivalence.duration.toString(), megaByteTotal, computedEquivalence.kWhTotal.toString(), computedEquivalence.gCO2Total.toString()]);
}
...
computeEquivalenceFromStatsItem = async (stats) => {
    let res = {};
    res.duration = await browser().storage.local.get('duration');
